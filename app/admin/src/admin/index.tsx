...
import { AccountCircle, EmojiEvents, Event, Face, Leaderboard, Tune, WorkspacePremium } from '@mui/icons-material';
...
const App = () => (
  <Admin basename={APP_BASENAME} dataProvider={apiProvider} authProvider={authProvider} loginPage={LoginPage} requireAuth>
    ...
    <Resource name="events" {...events} icon={Event} recordRepresentation={(record) => record.title} />
    <Resource name="prizes" {...prizes} icon={EmojiEvents} recordRepresentation={(record) => record.title} />
    <Resource name="awards" {...awards} icon={WorkspacePremium} recordRepresentation={(record) => `${record.prizeTitle} - $${record.value}`} />
    <Resource name="results" {...results} icon={Leaderboard} recordRepresentation={(record) => `${record.eventTitle}`} />
  </Admin>
);
